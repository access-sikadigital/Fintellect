  ...
  return (
    ...
    >
      ...
      <div className="container-wide section-y relative z-10">
        ...
        >
          {/*
            One ground for all six. These used to alternate — every third tile
            inverted to sand — which read as two different components rather
            than one set of six. They all take the sand card now, so the icon
            ring, heading and body no longer need to switch with it.
          */}
          {reasons.map((r) => (
            <article
              key={r.title}
              className="group flex h-full flex-col rounded-panel border border-sand bg-sand p-8 transition-colors duration-500 hover:border-clay-60"
            >
              ...
              <h3 className="type-title mt-6 text-[clamp(1.75rem,2.2vw,2.125rem)] text-forest">
                {r.title}
              </h3>
              <p className="type-body mt-3 text-forest/70">{r.body}</p>
            </article>
          ))}
        </Reveal>
      </div>
